...
$ sum_(n=1)^oo a_n quad и quad sum_(n=1)^oo b_n $
...
_Признак Раабе._ Для положительного ряда $sum a_n$ определим величину $ R_n = n(1 - (a_(n+1))/(a_n)) $
...
+ Пусть теперь $R_n <= 1$ для достаточно больших $n$. Имеем:
  $ n(1 - (a_(n+1))/(a_n)) <= 1 <=> a_(n+1)/a_(n) >= 1/(1 + 1/n) = n/(n+1) = (1/(n+1))/(1/n) $
...
_Признак Раабе в предельной форме._ Пусть для положительного ряда $sum a_n$ $ lim_(n->oo) n(1 - (a_(n+1))/(a_n)) = R $
...
_Интегральный признак Маклорена-Коши._ Пусть ряд можно представить в виде

$
  sum_(n=1)^oo a_n eq.triple sum_(n=1)^oo f(n) 
$
где $f(n)$ --- значение при $x = n$ непрерывной положительной монотонно убывающей функции $f(x)$ на полуинтервале $[1, +oo)$.

Тогда ряд $sum a_n$ сходится тогда и только тогда, когда сходится интеграл $ int_1^oo f(x) d x #[сходится] <=> sum_(n=1)^oo a_n #[сходится] $

_Доказательство._ $square$ Рассмотрим произвольную первообразную функцию $F(x)$ для $f(x)$ на полуинтервале $[1, +oo)$:

$
F(x) = int_1^x f(t) d t
$
Ясно, что $0 < f(x) = F'(x)$, поэтому $F(x) arrow.tr$. Поскольку $f(x)$ монотонно убывает, то для $x: n <= x <= n + 1$:

$
  a_(n+1) = f(n+1) <= f(x) <= f(n) = a_n
$
$
  a_(n+1) <= int_n^(n+1) f(t) d t <= a_n
$

Мы получиили оценку для общего члена такого ряда:

$
  sum_(n=1)^oo int_n^(n+1) f(t) d t
$

Его $n$-я частичная сумма равна 

$
  s_n = int_1^(n+1) f(t) d t = F(n+1) - F(1) ~_("сход.") F(n+1)
$
Итак, проанализируем предел 
$ L = lim_(x->oo) F(x) $

- Если $L < oo$, то ряд $sum a_(n+1) $ сходится по первой теореме сравнения как меньший либо равный сходящемуся ряду. И из сходимости $sum a_(n+1)$ следует сходимость $sum a_n$.

- Если $L = oo$, то ряд $sum a_n$ расходится по первой теореме сравнения как больший либо равный расходящемуся ряду.

Таким образом, ряд $sum a_n$ сходится тогда и только тогда, когда сходится интеграл $int_1^oo f(x) d x$. $square.filled$

==== Признак Куммера

#theorem[
  *Теорема(признак Куммера).* Пусть $a_n >= 0$ и произвольная последовательность ${c_n}$ такова, что ряд $sum 1/c_n$ _расходится._  Если $exists N: forall n > N:$
  $
    K_n := c_n a_n/(a_(n+1)) - c_(n+1) >= delta > 0,
  $
  то ряд $sum a_n$ сходится. Если же $K_n < 0$ для $n > N$, то ряд $sum a_n$ расходится. 

  В пределельной форме: 
  $
    K := lim_(n-> oo) c_n a_n/(a_(n+1)) - c_(n+1),
  $
...
+ При $c_n = 1$ получается признак Даламбера,

+ при $c_n = n$ --- признак Раабе,

+ При $c_n = n ln n$ --- признак Бертрана.

==== Признак Гаусса

#theorem[*Теорема(признак Гаусса).* Пусть дан положительный ряд $sum a_n$ и ограниченная последовательность ${c_n}$. Если отношение представимо в виде($epsilon > 0$)
$
  a_n / a_(n+1) = alpha +  beta/n + c_n /n^(1 + epsilon), 
$ 
то 
...
+ если $alpha = 1$, то 
  + при $beta > 1$ ряд $sum a_n$ сходится,

  + при $beta <= 1$ ряд $sum a_n$ расходится.
]

==== Признак конденсации Коши

#theorem[
  *Теорема(признак сгущения Коши).* Пусть последовательность $f(n)$ монотонно убывает и неотрицательна. Тогда ряды
  $
    sum_(n=1)^oo f(n) quad и quad sum_(n=1)^oo 2^n f(2^n)
  $
...
#theorem[*Теорема(признак Ермакова).* Пусть функция $f(x)$ положительна и монотонно убывает и 
$
  lambda := lim_(x->oo) (e^x f(e^x))/f(x),
$
+ Если $lambda < 1$, то ряд $sum f(n)$ сходится

+ если $lambda > 1$, то ряд $sum f(n)$ расходится. 
]

=== Сходимость произвольных рядов

Пусть задан ряд $sum a_n$, где члены $a_n$ имеют произвольные знаки. Вопрос его сходимости сводится к сходимости последовательности частичных сумм ${s_n}_(n=1)^oo$:

$ s_1, s_2, ... s_n, s_(n+1), ... s_(n+m), .. $

Ряд $sum a_n$ сходится тогда и только тогда, когда последовательность ${s_n}$ фундаментальна, то есть

$
  forall epsilon > 0 space exists N: forall n > N, forall m in NN: abs(s_(n+m) - s_n)< epsilon
$
...
_Доказательство._ $square$ Из абсолютной сходимости мы знаем, что начиная с некоторого номера и для всех $m in NN$:
$ forall epsilon > 0: space abs(sum_(i=0)^m abs(a_(n+i))) < epsilon $

Поскольку подмодульное выражение неотрицательно, то отбрасываем модуль:

$ sum_(i=0)^m abs(a_(n+i)) < epsilon $

Но с другой стороны, используем неравенство треугольника:

$
  abs(sum_(i=0)^m a_(n+i)) <= sum_(i=0)^m abs(a_(n+i)) < epsilon
$
Значит, ряд $sum a_n$ сходится. $square.filled$

Кроме того, если ряд $sum a_n$ сходится абсолютно, то ряды 
$ sum_(k=1)^oo b_k " и " sum_(m=1)^oo c_m, $ 


где $b_k$ - положительные члены ряда $sum a_n$ в порядке следования, а $c_m$ - модули отрицательных членов ряда $sum a_n$ в порядке следования, _сходятся._ И имеет место 

$
  sum_(n=1)^oo a_n = sum_(k=1)^oo b_k - sum_(m=1)^oo c_m
$
...
Ряд $sum a_n$ называется знакопеременным, если $forall n in NN: space a_n dot a_(n+1) < 0$. 

Не умаляя общности, можно давать такое определение для всех $n in NN$, так как конечное число членов ряда можно отбросить и перенумеровать оставшиеся с сохранением сходимости.

То же самое можно написать и так:

 $ sum_(n=1)^oo a_n = sum_(n=1)^oo (-1)^n c_n, $ где $c_n > 0$.

_Признак Лейбница._ Пусть ряд $sum a_n$ является знакопеременным. Если все члены ряда убывают по абсолютной величине и стремятся к нулю, то ряд $sum a_n$ сходится.

_Доказательство._ $square$ Пусть $c_n = |a_n|$, и $S_(2m)$ - частичная сумма чётного порядка:

$ S_(2m) = c_1 - c_2 + c_3 - ... + c_(2m-1) - c_(2m) $

$ S_(2m) = (c_1 - c_2) + (c_3 - c_4) + ... + (c_(2m-1) - c_(2m)) $

Поскольку $c_i > c_(i+1)$, то $S_(2m)$ с ростом $m$ возрастает.

$ S_(2m) = c_1 - (c_2 - c_3) - ... - (c_(2m-2) - c_(2m-1)) - c_(2m) $
