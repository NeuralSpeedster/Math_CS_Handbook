...
=== Обобщённая теорема Коши и интегральная формула Коши

#theorem([
#emoji.book.red _Теорема (б/д)._ Пусть $G$ --- ограниченная область с кусочно-гладкой положительно ориентированной границей $gamma$, а функция $f$ регулярна в $G$ и непрерывна на $overline(G)$. Тогда
...
#theorem([
#emoji.book.red _Теорема._ Пусть $G$ --- ограниченная область с кусочно-гладкой положительно ориентированной границей $gamma$, а функция $f$ регулярна в $G$ и непрерывна на $overline(G)$. Тогда $forall z in G$ выполняется формула Коши:

$
  f(z) = frac(1, 2 pi i) int_gamma (f(zeta) d zeta) / (zeta - z).
$
])

_Доказательство._ $square$ Если $z in G$, то существует такой радиус $r_0 > 0$, что круг $overline(B)_(r_0) (z) subset G$. Рассмотрим контур $gamma_r = {zeta: |zeta - z| = r}$, ориентированный против часовой стрелки. По обобщённой теореме Коши интеграл по замкнутому контуру $gamma$ равен сумме интегралов по контурам $gamma$ и $- gamma_r$:

Пусть $0 < r < r_0$ и $G_r = G \\ overline(B)_r (z)$. Тогда $f(zeta) / (zeta - z)$ регулярна в $overline(G_r)$, и по интегральной теореме Коши:

$
  int_(partial gamma_r) f(zeta) / (zeta - z) d zeta = int_(Gamma) (f(z))/(zeta - z) d zeta + int_(gamma_r-) (f(zeta)) / (zeta - z) d zeta.
$
