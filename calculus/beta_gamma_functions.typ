...
+ Значения в положительных полуцелых аргументах. $forall n in NN_0$:
  ...
  _Доказательство._ $square$ Используем $Gamma(1/2) = sqrt(pi)$.
...
+ Формула умножения. $forall x, n > 0$:
  $ Gamma(n x) = (2 pi)^(1/2) n^(n x - 1/2) product_(k=0)^(n-1) Gamma(x + k/n). $
  + В частности, для $n = 2$(формула удвоения Лежандра):
  $ Gamma(2 x) = (2 pi)^(-1/2) 2^(2 x - 1) Gamma(x) Gamma(x + 1/2). $

+ Представление Вейерштрасса. $forall x > 0$:
  $
    Gamma(x) = e^(-gamma x) / x product_(n=1)^oo (1 + x/n)^(-1) e^(x/n),
  $
где $gamma$ --- это константа Эйлера-Маскерони, определяемая как
...
$
Поскольку границы внутреннего интеграла не зависят от $phi$, то можно считать данную запись произведением двух отдельных интегралов! Но интеграл по $phi$ есть бета-функция, а интеграл по $r$ --- гамма-функция. Окончательно имеем:
...
_Доказательство._ $square$ 
Запишем, используя связь бета-функции и гамма-функции, значение $Beta(1/2, 1/2)$:
...
$ Gamma(1/2) = 2 int_0^oo e^(-x^2) d x = int_(-oo)^oo e^(-x^2) d x. space square.filled $
...
$
  psi(z) = (d ln(Gamma(z))) / (d z) = (Gamma '(z))/(Gamma(z)).
$
Производная гамма-функции равна
$
  Gamma ' (z) = int_0^oo t^(z-1) e^(-t) (ln t) d t,
$
следовательно, 
$
  Gamma ' (1) = int_0^oo e^(-t) ln t d t = -gamma,
$
откуда имеем 
#theorem[*Предложение.* $ psi(1) = (Gamma ' (1))/Gamma(1) = -gamma. $ ]
...
$ psi(z + 1) = (Gamma ' (z + 1))/(Gamma(z + 1)) = (Gamma(z) + z Gamma ' (z))/(z Gamma(z)) = 1/z + psi(z) 
$
#theorem[*Предложение.* $
psi(z + 1) = 1/z + psi(z). 
$ ]
