#import "../util.typ": *
#set math.equation(numbering: "(2.1)", supplement: none)
...
_Определение._ Величина $arg(z) = phi$, где $phi in (-pi;pi]$ --- ориентированный угол между радиус-вектором $z$ и положительным направлением оси абсцисс называется _аргументом комплексного числа_. Аргумент числа $(0, 0)$ не определён.
...
$forall z = r(cos phi + i sin phi) in CC, n in NN$: 
$
...
Здесь мы снова использовали (@complex_trig_mul). Таким образом, формула верна для $n + 1 =>$ она верна $forall n in NN$. $square.filled$


Умножение $z_1 = r_1(cos phi_1 + i sin phi_1)$ на $z_2 = r_2(cos phi_2 + i sin phi_2)$ задаёт композицию поворота $R_o^phi_2$ и гомотетии $H_O^r_2$ точки $z_1$  на плоскости $CC$. Полученное преобразование $RR^2 -> RR^2$ называется _поворотной гомотетией:_ $H_O^(r_2, phi_2) = H_O^r_2 circle.small R_O^phi_2$.
