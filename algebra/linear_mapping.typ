...
*Определение.* Пусть даны два векторных пространства $V$ и $W$ над полем $K$. Отображение $cal(A): V -> W$ называется линейным, если выполнены два условия:

+ $cal(x + y) = cal(x) + cal(y)$,  $forall x, y in V$.

+ $cal(lambda dot x) = lambda dot cal(x)$,  $forall lambda in K, x in V$.



*Определение.* Матрицей $A$ линейного отображения $cal(A)$ называется матрица $m times n$, такая, что

 $ A^((j)) = cal(A) (e_i), forall j = 1, ..., n, $

 где ${e_1,...e_n}$ --- базис в $V$. В $R^n$ можно взять например орты $e_i$.
...
+ Сложение. Пусть $cal(A), cal(B): V -> W$, тогда $forall x in V$
  $
    cal(C) = cal(A) + cal(B) <=> cal(C) (x) = cal(A) (x) + cal(B) (x)
  $
  Здесь $cal(C)$ отображает $V -> W$.

+ Умножение на число. Пусть $cal(A): V -> W$ и $lambda in K$, тогда $forall x in V$:
  $
    cal(C) = lambda dot cal(A) <=> cal(C) (x) = lambda dot cal(A) (x)
  $
  Здесь $cal(C)$ отображает $V -> W$.

+ Умножение линейных отображений. Пусть $A: V -> W$ и $B: U -> V$. Тогда $cal(C): U -> W$:

  $
    cal(C) = cal(A) dot cal(B) <=> cal(C) (x) = cal(A) (cal(B) (x)), forall x in U
  $
...
+ Умножение на число. Пусть $A$ - матрица $m times n$ и $lambda in K$, тогда для всех столбцов $forall j = 1, ..., n$:
  $
...
Пусть $cal(A): K^n -> K^m$ и $ x = vec(x_1, dots.v, x_n) in K^n $
тогда
$
  y = vec(y_1, dots.v, y_n) in K^m 
$
...
_Определение._ Отображение $cal(E): W -> V$ называется тождественным,
если 
$
  cal(E) (x) = x, forall x in W.
$ 
...
Основное свойство тождественного отображения $forall cal(A): V -> W$

$
  cal(A) dot cal(E) = cal(A)
$
Аналогично, $forall cal(B): U -> V:$
$ 
