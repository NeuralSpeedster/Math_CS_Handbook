...
#theorem([
#emoji.book.red _Теорема._ Ранг произвольной матрицы равен наибольшему порядку
её ненулевого минора. 
]
)

_Доказательство._ $square$ Пусть $r$ --- наибольший порядок ненулевого 
минора $M_(i_1, ..., i_r)^(j_1, ..., j_r) != 0$ матрицы $A$. Переставим строки и столбцы в матрице $A$.
От этого миноры могут лишь поменять знак, но не равенство или неравенство нулю.
А именно переставим так, чтобы $M_(1, ..., r)^(1, ..., r) != 0$, --- минор занимал первые 
$r$ строк и $r$ столбцов. Тогда $M_(1, ..., r)^(1, ..., r) = det overline(A) != 0$. Пусть 
$
  overline(A)_i = (a_(i 1), ..., a_(i r))
$
Матрица $overline(A)$ невырождена $=>$ её строки линейно независимы, значит ${overline(A)_1, ..., overline(A)_r}$
образуют базис в $RR^n =>$ $forall i > r space exists! (lambda_1, ..., lambda_r):$
$
  overline(A)_i = lambda_1 overline(A)_1 + ... + lambda_r overline(A)_r
$
Так как ${overline(A_1), ..., overline(A_r)}$ линейно независимы, то линейно независимы 
и строки ${overline(A_1), ..., overline(A_r)}$. Если бы это было не так, то $exists mu_1,...,mu_r !=0:$
$
  mu_1 overline(A)_1 + ... + mu_r overline(A)_r = 0
$
что неверно.

$forall i, j > r$ Рассмотрим $M_(1, ..., r, i)^(1, ..., r, j)$ --- он называется окаймляющим
минором, и он обязательно равен нулю. Рассмотрим соответствующую матриу $overline(overline(A))$.
Её строки линейно независимы, а ${overline(overline(A))_1, ..., overline(overline(A))_(r+1)}$
уже линейно зависимы.
$
  overline(overline(A))_(r+1) = lambda'_1 overline(overline(A))_1 + ... + lambda'_r overline(overline(A))_r
$
Поэтому для более коротких строк верно
$
  overline(A)_(r+1) = lambda'_1 overline(A)_1 + ... + lambda'_r overline(A)_r
$
В силу единственности выражения строки в базисе, 
$
  lambda'_1 = lambda_1, ..., lambda'_r = lambda_r
$
Следовательно $forall i=1,...,n$:
$
  a_(i j) = lambda_1 a_(1 j) + ... + lambda_r a_(r j) => A_i = lambda_1 A_1 + ... + lambda_r A_r
$
И поэтому ${A_1, ..., A_r}$ образует базис системы строк матрицы $A$, поэтому ранг матрицы $A$
равен $r$.
$square.filled$
