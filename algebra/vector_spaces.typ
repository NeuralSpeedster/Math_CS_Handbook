...
Пусть $(e_1, ..., e_n)$ и $(e'_1, ..., e'_n)$ --- два базиса в $V$, причём $(e'_1, ..., e'_n) = (e_1, ..., e_n) dot C$.

_Определение._ Матрица $C$ называется матрицей перехода от базиса $(e_1, ..., e_n)$ к базису $(e'_1, ..., e'_n)$.

В $j$-м стоблце матрицы $C$ стоят координаты вектора $e'_j$ в базисе $(e_1, ..., e_n)$.

#theorem([_Предложение._ Пусть $v in V$ и $v = x_1 e_1 + ... + x_n e_n$ и $v = x'_1 e'_1 + ... + x'_n e'_n$ --- вектор $v$ выражается в двух разных базисах, причём 
$(e'_1, ..., e'_n) = (e_1, ..., e_n) dot C$. Тогда 
$
  vec(x_1, dots.v, x_n) = C dot vec(x'_1, dots.v, x'_n)
$
])
_Доказательство._ $square$ С одной стороны, $v = (e_1, ..., e_n) dot vec(x_1, dots.v, x_n)$. С другой стороны, 
$
v = (e'_1, ..., e'_n) dot vec(x'_1, dots.v, x'_n) = dot (e_1, ..., e_n) dot C dot vec(x'_1, dots.v, x'_n).
$
Так как $(e_1, ..., e_n)$ --- линейно независимы, то вектор представленный их линейной комбинацией, представляется так единственным способом, поэтому 
$
  vec(x_1, dots.v, x_n) = C dot vec(x'_1, dots.v, x'_n) space square.filled
$
...
_Определение._ Подпространством векторного пространства $V$ называется непустое множество векторов $U subset.eq V$, которое удовлетворяет двум требованиям замкнутости:

+ $forall x, y in U: x + y in U$

+ $forall x in U, lambda in RR: lambda dot x in U$
...
+ Линейная оболочка системы векторов является подпространством. Пусть $S subset.eq V$. Линеная оболочка системы $S$ обозначается $<S>$:
  $
    <S> =_("def") {v = lambda_1 v_1 + ... + lambda_n v_n | lambda_1, ..., lambda_n in RR, v_1, ..., v_n in S}
  $
  + $<S>$ является наименьшим подпространством, содержащим $S$.
  + Если $B$ --- базис $S$, то $B$ --- базис $<S>$.
  + $dim <S> = op("rk") S$.
...
$
Тогда множество её решений $U subset.eq RR^n$ --- подпространство, причём
$
  dim U = n - op("rk") A.
$
])
_Доказательство._ $square$ Докажем, что $U$ является подпространством.
+ В силу однородности системы, $accent(0, ->) = (0, ..., 0) in U$
+ Пусть есть два решения системы: $y = (y_1, ..., y_n)$ и   $z = (z_1, ..., z_n)$. Тогда $forall i=1,...,m:$
  $
    a_(i 1) y_1 + a_(i 2) y_2 + ... + a_(i n) y_n = 0, \
    a_(i 1) z_1 + a_(i 2) z_2 + ... + a_(i n) z_n = 0.
  $
  Если мы сложим эти два равенства, то получим:
  $
    a_(i 1) (y_1 + z_1) + a_(i 2) (y_2 + z_2) + ... + a_(i n) (y_n + z_n) = 0.
$
  Тогда $y + z in U$(тоже является решением системы).

+ Пусть $lambda in RR$. Если $y in U$, то, умножая все уравнения на $lambda$, получаем $forall i=1,...,m:$
$
  a_(i 1) (lambda y_1) + a_(i 2) (lambda y_2) + ... + a_(i n) (lambda y_n) = 0.
$
Таким образом, $y in U => lambda dot y in U =>$ $U$ является подпространством. $square.filled$
