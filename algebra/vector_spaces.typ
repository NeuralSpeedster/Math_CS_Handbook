...
+ Основная лемма о линейной зависимости. Пусть $S = {v_1, ..., v_n}$ и $T = {w_1, ..., w_m}$, причём система $S$ линейно выражается через систему $T$ и $n > m$. Тогда система $S$ линейно зависима.
...
+ Если $S = empty$ или $S = {0} => B = {0}$. Иначе в системе есть ненулевой вектор $v_1 in S => {v_1}$ линейно независима. 
  ...
  $
  Отсюда $=> forall {v_1, ..., v_k}$ линейно выражается через ${e_1, ..., e_n}$. 
